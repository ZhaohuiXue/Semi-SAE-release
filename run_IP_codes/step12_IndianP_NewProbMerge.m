clear;pack;clc;
load IndianPHSPFineT5.mat;
load IndianPSPFFineT6.mat;
load IndianP_train_test.mat;
%TrainingPointsCell, TestPointsCell

M = 145; N = 145; Tregion = 4;
numClasses = uint8(16);
...
% computers.
nBand = size(im_IP10,3);
im_IP10 = reshape(im_IP10,[],nBand);
...
for k1=1:numClasses 
    ...
    for k2=1:temp.NumObjects
        ...
        if regionPixelNum(jj)>1
            regionMean(jj,:) = mean(im_IP10([PixelIdxList{jj,1}],:));
        else
            regionMean(jj,:) = im_IP10([PixelIdxList{jj,1}],:);
        end
        regionLabels(jj) = k1;
    end
end
...
save(sprintf('%s%s%s','IP_final_map_',datestr(now,30),'.mat'),'imt2');